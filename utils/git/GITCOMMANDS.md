...
- [⚙️ Setting Up Git](#️-setting-up-git)
...
- **Local scope** (current repository only):

  ```bash
  git config --local user.email [your_mail_here]
  ```

- **Global scope** (applies to all repositories for your user):

  ```bash
  git config --global user.email [your_mail_here]
  ```

- **System scope** (system-wide):
...
- **Cherry-pick a commit (reapply a lost commit):**
...
- **Bisect workflow to find a bad commit:**
...
- **Replace author email throughout history (use with extreme caution):**
...
- **Remove a file from entire history (use with extreme caution):**
...
😄 We hope this guide has been helpful. If you have feedback or want to contribute more information, please open a pull request or an issue!
